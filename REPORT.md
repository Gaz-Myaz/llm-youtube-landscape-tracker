...
- Configurable transcript provider chain: cache, `youtube-transcript-api`, `yt-dlp`, then local Whisper audio transcription fallback.
...
4. Local Whisper transcription from downloaded audio.
...
This is no longer only a subtitle fallback chain. If YouTube has no usable subtitles and `yt-dlp` cannot expose a subtitle track, the worker downloads audio and runs local `faster-whisper` transcription before skipping the video. Some videos still fail all providers when the source is a live event, otherwise unplayable, or YouTube blocks access.
...
then the dashboard was generated by the real transcript-backed pipeline.
...
then the dashboard was generated by the real transcript-backed pipeline with Gemini structured extraction. The current live metadata is:
...
```json
{
	...
	"estimated_cost_usd": 0.107155,
	"last_successful_run_at": "2026-05-10T14:43:39Z"
}
```

For that run, the worker inspected up to 10 channels and 50 candidate videos, attempted Gemini extraction for 48 caption-backed candidates, published 34 videos, filtered 14 caption-backed videos as outside the configured LLM landscape rules, and marked the data `partial` because 2 videos still failed transcript acquisition even after `yt-dlp` and Whisper fallback.

New exports write `token_usage`, `cost_rates`, and `estimated_cost_usd` into `run-metadata.json`. Token usage is extracted from OpenAI-compatible, Gemini, or Anthropic usage fields when the provider returns them, and cost is estimated from configurable per-million-token rates. The provider billing dashboard remains the source of truth for free-tier handling, rounding, taxes, and invoices.

During local testing and the current production workflow, the real export successfully produces caption-backed snapshots, but YouTube can still temporarily block transcript access or expose videos that are live/unplayable. The worker now reports compact skip reasons, uses the transcript cache, throttles uncached requests, falls back to `yt-dlp` subtitle extraction, and then tries local Whisper transcription before the video is counted as failed.
...
- fallback from subtitle providers to Whisper audio transcription
- max-channel run limiting
...
- YouTube caption access can still be rate-limited or blocked, though cache, delay, `yt-dlp`, and Whisper fallback reduce this risk.
- Some videos still fail transcript acquisition when the source is live, unplayable, or YouTube blocks both subtitle and audio access.
...
1. Harden transcript acquisition further for live events, blocked videos, and other cases where cache, `yt-dlp`, and Whisper still cannot recover a usable transcript.
